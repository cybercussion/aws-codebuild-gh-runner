...
resource "aws_codebuild_project" "github_runner" {
  ...
  source {
    ...
    #   env:
    #     variables:
    #       RUNNER_LABEL: "self-hosted-${var.account_name}"
    #   phases:
    #     build:
    #       commands:
    ...
    #         - GITHUB_TOKEN=$(aws ssm get-parameter --name /github/codestar/token --with-decryption --query Parameter.Value --output text)
    #         - ./config.sh --url https://github.com/${var.github_repo} --token $GITHUB_TOKEN --labels "$RUNNER_LABEL"" --unattended
    #         - ./run.sh
    # EOT
